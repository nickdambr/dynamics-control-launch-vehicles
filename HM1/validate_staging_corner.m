...
%  instead of the outer parametric sweep of main_task4.m. The staging time is
%  appended as a fifth shooting unknown and the corner condition as a fifth
%  residual; the augmented solve must reproduce the swept optimum (ts ~ 0.336).
%
%  Corner condition (report, Task 4):
%     eta*[lam_m(tf) - lam_m(ts)] = c*||lam_v(ts)||*(1/m_plus - 1/m_minus)
%  with  m_minus = 1 - Q*ts,  m_plus = 1 - (1+eta)*Q*ts.
%
%  Reuses ode_burn.m (same powered-flight RHS as main_task4.m); run from HM1/.
...
%% Cautionary check: the burnout reference lam_m(tf) is essential.
...
% Integrate stage 1 [0, ts], jettison stage-1 structure, stage 2 [ts, tf].
% Returns terminal state zf and the pre-staging state zs_minus (= state at ts^-).
    pp = p;
...
% Fixed-ts inner BVP (4 unknowns), same formulation as main_task4.m.
...
% Augmented residual: inner BVP (4) + corner condition (1). 5 unknowns.
...
% Same as shooting_corner but with the (incorrect) un-referenced corner term
% eta*lam_m(ts) instead of eta*[lam_m(tf)-lam_m(ts)]. Used only to show that
% dropping the burnout reference misplaces the optimum.
    lam_vx0 = w(1); lam_vy0 = w(2); lam_y = w(3); tf = w(4); ts = w(5);
    if tf <= ts || tf > 2 || ts <= 0 || ts > 1/(p.Q*(1+p.eta)) - 1e-3
        res = 1e6*ones(5,1); return;
    end
