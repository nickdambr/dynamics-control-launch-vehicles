...
Before tackling the ascent BVP, the single-shooting machinery is validated on a classical minimum-energy problem whose solution is known in closed form---the \emph{rocket sled}~\cite[Ch.~3]{zavoli2013}. A unit point mass moves on a frictionless rail under a force $u$,
\begin{equation}
...
\begin{equation}
    ...
    u^*(t) = \tfrac{3}{4}\,(1 - t),
\end{equation}
which yields $v(t) = \tfrac{3}{4}\bigl(t - t^2/2\bigr)$ and $r(t) = \tfrac{3}{4}\bigl(t^2/2 - t^3/6\bigr)$, satisfying $r(2) = \tfrac{1}{2}$ and $v(2) = 0$ exactly. Running the same \texttt{ode45}\,+\,\texttt{fsolve} shooting code on this two-unknown problem recovers $(\lambda_{r,0}, \lambda_{v,0}) = (\tfrac{3}{2}, \tfrac{3}{2})$ and drives the terminal residual to zero to solver tolerance, validating the implementation before it is applied to the ascent BVP. The rocket sled is also the textbook illustration of shooting fragility: its convergence map shows sizeable regions of the initial-costate plane from which Newton iterations diverge~\cite[Ch.~3]{zavoli2013}, motivating the continuation strategy of Section~\ref{sec:shooting}.
