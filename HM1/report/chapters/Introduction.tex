...
By the PMP, the control $\varphi^*$ maximizes $\mathcal{H}$ pointwise:
\begin{equation}
    ...
    \label{eq:bilinear}
\end{equation}
This is the \textbf{bilinear-tangent (linear-tangent) law}: since $\lambda_{v_y}$ is linear in $t$ (Eq.~\ref{eq:lvy}) and $\lambda_{v_x}$ is constant, $\tan\varphi^*$ varies linearly with time. At the optimal angle,
\begin{equation}
    \lambda_{v_x}\cos\varphi^* + \lambda_{v_y}\sin\varphi^* = \|\boldsymbol{\lambda}_v\|,
\end{equation}
and the optimal Hamiltonian simplifies to
\begin{equation}
