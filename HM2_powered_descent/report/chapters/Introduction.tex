...
This report presents the solution to Homework~2 of the course \emph{Dynamics and Control of Launch Vehicles}: the minimum-fuel powered descent and pinpoint soft landing of a reusable launch vehicle. While Homework~1 tackled an ascent problem via the indirect approach (Pontryagin Maximum Principle plus shooting), this work adopts a \emph{direct transcription}: the optimal control problem is approximated by a finite-dimensional Nonlinear Programming (NLP) problem through trapezoidal collocation, which is then solved by Sequential Quadratic Programming (SQP).
