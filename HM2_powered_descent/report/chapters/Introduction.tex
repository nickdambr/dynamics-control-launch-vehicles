...
This report presents the solution to Homework~2 of the course Dynamics and Control of Launch Vehicles: the minimum-fuel powered descent and pinpoint soft landing of a reusable launch vehicle. Homework~1 tackled an ascent problem with the indirect approach (Pontryagin Maximum Principle plus shooting); here I use a \textbf{direct transcription} instead. The optimal control problem is approximated by a finite-dimensional Nonlinear Programming (NLP) problem through trapezoidal collocation, which is then solved by Sequential Quadratic Programming (SQP). The optional Task~2 adds a Zero-Order Hold transcription and a Successive Convexification (SCvx) loop whose inner sub-problem is solved both as a generic NLP and as a Second-Order Cone Program.

The powered-descent problem models the final phase of flight of a reusable rocket performing a "divert and soft land" manoeuvre, of the kind flown by the SpaceX Falcon~9 first stage and by Mars precision-landing concepts. The vehicle must reach a designated landing pad with zero residual velocity, on minimum propellant, while respecting thrust bounds, a glide-slope corridor, and an altitude constraint.

\paragraph{Why a direct method.} I picked a direct transcription over the indirect route of Homework~1 for a few reasons. Indirect methods optimize then discretize: they require the analytical costate equations, the transversality conditions, and a guess for the non-physical initial costates. A direct method discretizes then optimizes, transcribing the trajectory into an NLP that a general-purpose solver handles without any of that machinery. Three things make this convenient here. Path constraints (glide-slope, thrust bounds) enter natively as algebraic inequalities. The converged NLP active set shows the constrained-arc structure of the optimum directly (here the saturated burns of the max--coast--max law). And the convergence radius is much wider, so a crude initial guess is enough. Betts shows that the direct route can even solve high-index differential--algebraic problems on which a hand-reduced indirect formulation fails~\cite[Sec.~4.12.7]{betts2010}. The price is a larger problem, and the discrete optimum need not satisfy the continuous necessary conditions exactly; the convex reformulations in Task~2 are the route to the real-time guidance that a dense NLP cannot provide~\cite{benedikter2022}.
...
\label{eq:OCP}
\end{equation}
The objective is written in \textbf{Mayer} form, as a pure terminal cost $J = -m(t_f)$. It could equally be cast in \textbf{Lagrange} form through the mass dynamics, $\int_0^{t_f}\!\|\mathbf{T}\|/c\,\mathrm{d}t = m_0 - m(t_f)$; the two are special cases of the \textbf{Bolza} objective, and a Lagrange integral cost can always be converted to Mayer form by appending one extra state~\cite[Sec.~4.4]{betts2010}. I use the Mayer form because the terminal mass is already a decision variable, so it adds nothing to the transcription, whereas the Lagrange form would carry one extra state at every node.
