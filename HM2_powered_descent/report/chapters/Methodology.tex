...
\begin{equation}
    ...
    \label{eq:trap_rule}
\end{equation}
The order of the rule follows from a Taylor expansion about $t_k$: with $\mathbf{x}_{k+1} = \mathbf{x}_k + \Delta t\,\mathbf{f}_k + \tfrac{1}{2}\Delta t^2\dot{\mathbf{f}}_k + O(\Delta t^3)$ and $\tfrac{\Delta t}{2}(\mathbf{f}_k + \mathbf{f}_{k+1}) = \Delta t\,\mathbf{f}_k + \tfrac{1}{2}\Delta t^2\dot{\mathbf{f}}_k + O(\Delta t^3)$, their difference---the local truncation error---is $\boldsymbol{\tau}_k = -\tfrac{1}{12}\Delta t^3\,\ddot{\mathbf{f}}_k + O(\Delta t^4) = O(\Delta t^3)$\footnote{Both expansions agree through $O(\Delta t^2)$; carrying them one order further makes the $-\tfrac{1}{12}$ explicit. With $\dot{\mathbf{x}} = \mathbf{f}$, the exact solution is $\mathbf{x}_{k+1} = \mathbf{x}_k + \Delta t\,\mathbf{f}_k + \tfrac{1}{2}\Delta t^2\dot{\mathbf{f}}_k + \tfrac{1}{6}\Delta t^3\ddot{\mathbf{f}}_k + O(\Delta t^4)$, while $\mathbf{f}_{k+1} = \mathbf{f}_k + \Delta t\,\dot{\mathbf{f}}_k + \tfrac{1}{2}\Delta t^2\ddot{\mathbf{f}}_k + O(\Delta t^3)$ gives $\tfrac{\Delta t}{2}(\mathbf{f}_k + \mathbf{f}_{k+1}) = \Delta t\,\mathbf{f}_k + \tfrac{1}{2}\Delta t^2\dot{\mathbf{f}}_k + \tfrac{1}{4}\Delta t^3\ddot{\mathbf{f}}_k + O(\Delta t^4)$. The $\Delta t$ and $\Delta t^2$ terms cancel, so the leading difference is $\bigl(\tfrac{1}{6} - \tfrac{1}{4}\bigr)\Delta t^3\ddot{\mathbf{f}}_k = -\tfrac{1}{12}\Delta t^3\ddot{\mathbf{f}}_k$. Over a fixed horizon there are $N \sim 1/\Delta t$ steps, so the error per step, $O(\Delta t^3)$, accumulates to $N\,O(\Delta t^3) = O(\Delta t^2)$: the global order is one below the local order, as for any stable scheme.}; summed over the $O(1/\Delta t)$ intervals this yields a globally second-order, $O(\Delta t^2)$, method~\cite[Eq.~(4.56) and p.~133]{betts2010}. The rule is implicit ($\mathbf{f}_{k+1}$ depends on $\mathbf{x}_{k+1}$) and A-stable: applied to the scalar test equation $\dot{x} = \lambda x$ it gives the amplification factor $x_{k+1}/x_k = (1 + \lambda\Delta t/2)/(1 - \lambda\Delta t/2)$, whose modulus stays below unity for every $\operatorname{Re}(\lambda) < 0$ (the Crank--Nicolson scheme), so stiff modes impose no step-size restriction.\footnote{Writing $z = \lambda\Delta t = a + \mathrm{i}b$, the amplification factor $R$ has $|R|^2$ with numerator $(1+a/2)^2 + (b/2)^2$ and denominator $(1-a/2)^2 + (b/2)^2$; their difference is $(1+a/2)^2 - (1-a/2)^2 = 2a$, negative whenever $a = \operatorname{Re}(\lambda\Delta t) < 0$, so $|R| < 1$ for every $\lambda$ with negative real part. As $|z| \to \infty$, $R \to -1$ and $|R| \to 1$: the rule is A-stable but not L-stable, so stiff modes stay bounded but are not damped within one step.}
...
The NLP multipliers carry physical meaning: as the mesh is refined they converge to scaled values of the continuous costates, the covector mapping property. For the trapezoidal transcription, the discrete multipliers $\boldsymbol{\alpha}_k$ of the path constraints map to the continuous path-constraint adjoint $\hat{\boldsymbol{\mu}}_k$ (distinct from the inequality multipliers $\boldsymbol{\mu}$ of Section~\ref{sec:kkt}) through~\cite[Sec.~4.11.2, Eqs.~(4.235)--(4.237)]{betts2010}
\begin{equation}
...
The NLP \eqref{eq:nlp} is solved with MATLAB's \texttt{fmincon} configured with the SQP algorithm~\cite[Ch.~1]{betts2010}. SQP iteratively constructs a quadratic-programming approximation of the KKT system: at each iterate $\mathbf{z}^{(k)}$ the search direction $\mathbf{d}^{(k)}$ is found by solving
\begin{equation}
\begin{aligned}
...
This quadratic subproblem is not an ad hoc model: it is one Newton step on the KKT system of Section~\ref{sec:kkt}. Collect the stationarity condition and the equality constraints into the nonlinear map $\mathbf{F}(\mathbf{z},\boldsymbol{\lambda}^{\mathrm{eq}}) = \bigl(\nabla_{\mathbf{z}}\mathcal{L},\; \mathbf{c}_{\mathrm{eq}}\bigr)$; applying Newton's method to $\mathbf{F} = \mathbf{0}$ gives a linear system for the increment $(\mathbf{d}^{(k)},\Delta\boldsymbol{\lambda}^{\mathrm{eq}})$ whose two blocks are exactly the stationarity of the QP objective and the linearised equality constraint above. The inequalities enter through which $\mathbf{c}_{\mathrm{ineq},j}$ the subproblem holds at equality at the current step, its active set. At convergence the QP multipliers are the NLP multipliers $(\boldsymbol{\lambda}^{\mathrm{eq}},\boldsymbol{\mu})$ of Section~\ref{sec:kkt}, the same ones that Section~\ref{sec:covector} maps to the continuous costates.

Forming the exact Hessian of the Lagrangian $\nabla^2_{\mathbf{z}\mathbf{z}}\mathcal{L}$ would need second derivatives of the dynamics; \texttt{fmincon} instead grows $\mathbf{H}^{(k)}$ from successive Lagrangian gradients by a damped BFGS update (Powell's modification), which keeps it positive definite so the subproblem stays a convex QP and $\mathbf{d}^{(k)}$ is a genuine descent direction. The price is a superlinear rather than quadratic local rate, for the benefit of needing only first derivatives.

Newton's step converges only near the solution, so from the cold start (states linearly interpolated between the endpoints, thrust at hover) it has to be globalised. The line search shrinks $\alpha^{(k)}$ until a merit function, typically the $\ell_1$ penalty $\phi(\mathbf{z}) = J(\mathbf{z}) + \rho\bigl(\|\mathbf{c}_{\mathrm{eq}}\|_1 + \|\max(\mathbf{0},\mathbf{c}_{\mathrm{ineq}})\|_1\bigr)$, decreases; folding the objective and the constraint violation into one scalar lets the iterate make monotone progress toward a feasible optimum, with the penalty $\rho$ raised when needed to keep $\mathbf{d}^{(k)}$ a descent direction of $\phi$. Once the active set settles and the iterate reaches the basin of the solution the local rate is superlinear, which keeps the iteration count modest at $N = 50$.
