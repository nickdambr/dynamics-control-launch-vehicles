...
max-$\bar q$ condition studied here. The sign of $\alpha_w$ is the one implied by
Eq.~\eqref{eq:plant}: $\alpha$ is the incidence measured against the
\emph{air-relative} velocity, so a lateral wind subtracts from the vehicle's own
$\dot z$. Note that no weathercock stability is available to relieve the load ---
with $A_6 > 0$ the centre of pressure sits \emph{ahead} of the centre of mass and
the aerodynamic moment is divergent, so an attitude-hold law is if anything load
aggravating (Task~1). The weak negative gains close the free integrator in
lateral position and keep the drift bounded; I therefore monitor both $z$ and
$\bar q\,\alpha$ in the gust simulations. The actual deflection follows from the actuator and
